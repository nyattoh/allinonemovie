...
- 各ドラフトは `drafts/作品名/draft_01/` 〜 `draft_10/` のディレクトリに個別の `screenplay.md` として保存されます。
- 10本のドラフトは、展開・結末・ギミック・演出などがすべて異なる内容となります。
- **AIが自動で10本のドラフトを評価し、上位3案を選出して提示します。**
- ユーザーは上位3案の中から最も優れたものを選択し、以降のリライトや映像化工程に進みます。

### 運用方法
1. 作品ごとに `drafts/作品名/` ディレクトリを作成します。
2. その配下に `draft_01` 〜 `draft_10` のディレクトリを用意します。
3. 各ディレクトリに異なるストーリー案（`screenplay.md`）を自動生成します。
4. **AIが10本のドラフトを自動評価し、上位3案を`drafts/作品名/draft_01`〜`draft_03`として提示します。**
5. 3案の中から1本を選択して次工程へ進みます。
