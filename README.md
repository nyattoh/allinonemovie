...
> 本システムは **main.yaml** の定義に従って、全ワークフロー（パラレルキャラバン・AI評価・分岐・推敲など）が自動実行されます。
...
## 重要事項
- **main.yamlに従ってワークフローが自動進行します。**
  - すべての工程（ヒアリング、草案生成、評価、推敲、分岐、映像化など）はmain.yamlの設定に基づき自動で進みます。
- **ディレクトリ作成・ファイル生成は自動で行われます。**
  - ユーザーが個別にディレクトリやファイルを手動作成する必要はありません。
  - 進行中のワークフローに応じて、必要なディレクトリ・ファイルが自動生成されます。
...
本システムでは「パラレルキャラバン」と呼ばれる仕組みを採用しています。

### パラレルキャラバンの概要
- 1つのプロンプト（例：ごじにゃの雨の日）に対して、**10本の異なるストーリー（ドラフト）を並列で自動生成**します。
- 各ドラフトは `drafts/作品名/draft_01/` 〜 `draft_10/` のディレクトリに個別の `screenplay.md` として自動生成されます。
- 10本のドラフトは、展開・結末・ギミック・演出などがすべて異なる内容となります。
- **AIが自動で10本のドラフトを評価し、上位3案を選出して提示します。**
- ユーザーは上位3案の中から最も優れたものを選択し、以降のリライトや映像化工程に進みます。

### 運用方法
- main.yamlの定義に従い、ワークフロー進行時に必要なディレクトリ・ファイルは**すべて自動生成されます**。
- ユーザーが個別にディレクトリやファイルを手動作成する必要はありません。
- 進行中のワークフローに応じて、AIが自動的にストーリー案や評価ファイルなどを生成します。

> **注意**：「パラレルキャラバン」はキャラクターのバリエーションではなく、ストーリー（脚本）のバリエーションを並列生成し、AIが自動評価・選出する仕組みです。
...
- **動作しない場合は「README.mdに従ってワークフローを実行してください」と入力ください。**
- **AIアシスタントはGPT系（例：GPT-4.0/4.1）を推奨します。他モデルでは一部機能が正しく動作しない場合があります。**
